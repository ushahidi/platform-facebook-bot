    ...
    public function startConversation(Request $request) 
    {
        ...
        // getting the bot-answer from database 
          ...
          if(!empty($answers) && $message_type !== 'location') {
            ...
            } else if($command === self::SEND || $command === 'send my report') {
                $this->reportingFlow($ongoingReport, $message, $recipient, $message_type);
            } else if($command !== self::ADD_IMAGE && $command !== self::LOCATION_RECEIVED && $command !== self::IMAGE_RECEIVED && $command !== self::IMAGE_LOCATION && $command !== self::ADD_IMAGE && $command !== self::ADD_LOCATION) {
